module WillPaginate
    ...
    def will_paginate(entries = @entries, options = {})
      ...
      if total_pages > 1
        options = options.symbolize_keys.reverse_merge(pagination_options)
				page, param, form = entries.current_page, options.delete(:param_name), options.delete(:form_name)
        ...
        visible.each do |n|
          ...
          unless n - prev > 1
            prev = n
            links << page_link_or_span((n != page ? n : nil), 'current', n, param, form)
          else
        ...
        links.unshift page_link_or_span(entries.previous_page, 'disabled', options.delete(:prev_label), param, form)
        links.push    page_link_or_span(entries.next_page,     'disabled', options.delete(:next_label), param, form)
  ...
  protected
    ...
    def page_link_or_span(page, span_class, text, param, form)
      unless page
        content_tag :span, text, :class => span_class
      else
        # page links should preserve GET parameters, so we merge params
				if form
					form_params = params[form.to_sym] || Hash.new
					link_to text, form.to_sym => form_params.merge({ param.to_sym => page} )
				else
