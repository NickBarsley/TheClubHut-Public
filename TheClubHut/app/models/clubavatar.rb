class Clubavatar < ActiveRecord::Base  
  # Image sizes
  IMG_SIZE = '"500x500>"'
  THUMB_SIZE = '"50x64>"'
  ...
  if ENV["RAILS_ENV"] == "test"
    ...
    URL_STUB = "/images/clubavatars"
    DIRECTORY = File.join("public", "images", "clubavatars")
  end
  
  def initialize(club, image = nil)
    @club = club
    @image = image
  ...
  def convert
    if ENV["OS"] =~ /Windows/
      ...
      "C:\\Program Files\\ImageMagick-6.4.1-Q8\\convert"
    else
      "/usr/bin/convert"
    end
  end
  ...
  def successful_conversion? 
    ...
    source = File.join("tmp", "#{@club.id}_full_size")
